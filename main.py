...
def ibd2sql(args: dict):
        ...
        if not is_windows:
            ...
            if not os.path.isfile(ibd2sdi_path):
                raise FileNotFoundError(
                    f'ibd2sdi({ibd2sdi_path}) not exist, output: {" ".join(res_arr)} '
                )
    ...
    for ibd_name in ibd_names:
        ...
        if sdi_data.get("dd_object_type") != "Table":
            logger.warning(
                f'unsupported sdi type: {sdi_data.get("dd_object_type")} for {ibd_name}'
            )
            continue
        ...
        for idx in indexes:
            ...
            if not use_elts:
                logger.warn(
                    f"invalid index found, no columns linked: {ibd_name}/{idx_name}"
                )
                continue
            builder.write(",\n\t")
            ...
            if idx_type == 1:
                ...
                raise ValueError(
                    f"unsupport index type: {idx_type} for {ibd_name}/{idx_name}"
                )
